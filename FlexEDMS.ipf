	...
	for(i=0;i<N_temps;i+=1)
		...
		if(StringMatch(temperature_option,"all") || StringMatch(temperature_name,temperature_option))
			...
			for(j=0;j<N_biases;j+=1)
				...
				FEDMS_AnalyzeTOFData(sample_name,temperature_name,carrier_type,bias_name)
				if(V_status==-1)
...
Function FEDMS_AnalyzeTOFData(sample_name,temp_name,carrier_type,bias_name)
	...
	String bias_name
	String original_folder = GetDataFolder(1)
	SetDataFolder root:FlexEDMS:$(sample_name):$("Time of Flight"):$(carrier_type):$(temp_name)
	Wave rejection_mask
	...
	if(rejection_mask[Measurement_index]==1)
...
Function FEDMS_LoadToFDataFolder()
	String original_folder = GetDataFolder(1)
